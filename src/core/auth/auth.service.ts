  ...
  async changePassword(
    ...
    try {
      ...
      let user = await this.prismaService.user.findFirst({
        where: { id: user_id, password: oldPasswordHash },
      });
      console.log('changePasswordAPIUser', user);
      ...
      if (user) {
        ...
        this.logger.verbose(
          `User ${userIdentity({ email })} updated its password`,
        );
        return user;
      } else {
        this.logger.error('There is no match user!');
